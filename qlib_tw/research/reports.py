...
def dump_report_frames(
    ...
    *,
    ...
    strategy_return = report_df["return"] - report_df["cost"] if "cost" in report_df.columns else report_df["return"]
    strategy_curve = (1 + strategy_return).cumprod()
    benchmark_curve = (1 + report_df["bench"]).cumprod()
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(strategy_curve.index, strategy_curve.values, label="Strategy (with cost)")
    ax.plot(benchmark_curve.index, benchmark_curve.values, label="Benchmark")
    ax.set_title("Strategy vs Benchmark Wealth Multiple")
    ax.set_xlabel("Date")
